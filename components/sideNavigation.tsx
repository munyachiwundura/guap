...
const SideNavigation: FunctionComponent = () => {
  ...
  return (
    ...
    { open && <>
      ...
      <Notifications/>
        <button onClick={()=> setOpen(false)} className='text-3xl mr-10 dark:text-white z-50 absolute top-6 right-6'><i className='bi bi-x-lg'></i></button>
    </>
      }
    <nav className="flex flex-col h-screen fixed top-0 left-0 p-4 items-center justify-between z-30 ">
        ...
        <button onClick={()=> setOpen(true)} className='text-3xl mr-10 dark:text-white'><i className='bi bi-bell'></i></button>
        </div>
